...
function App(): React.JSX.Element {
  ...
  const handleLogin = async (): Promise<void> => {
    ...
    try {
      await window.api.setStore('udemy_token', token)
                    ...
                    <div className="flex flex-col gap-2 relative">
                      ...
                      >
                        <span className="font-medium">
                          {qualityOptions.find((opt) => opt.id === appSettings.videoQuality)
                        ...
                        >
                          ...
                          <div className="relative">
                            <input
                              ...
                              className="sr-only"
                              checked={
                                appSettings[setting.id as keyof typeof appSettings] as boolean
                              }
                              onChange={(e) =>
                                setAppSettings((prev) => ({
                                  ...prev,
  ...
  return (
    <div className="relative flex flex-col items-center justify-center min-h-screen p-8 bg-slate-50 dark:bg-[#09090e] overflow-hidden transition-colors duration-500">
      ...
      <div className="relative bg-white/60 dark:bg-white/5 backdrop-blur-2xl border border-white/40 dark:border-white/10 rounded-[2rem] p-10 shadow-2xl dark:shadow-[0_8px_32px_0_rgba(0,0,0,0.36)] w-full max-w-md transition-all duration-500 z-10 before:absolute before:inset-0 before:bg-gradient-to-b before:from-white/40 dark:before:from-white/5 before:to-transparent before:rounded-[2rem] before:pointer-events-none">
        ...
        <div className="flex flex-col gap-6">
          <div className="relative group flex flex-col gap-2">
            <label
              className={`block text-sm font-semibold mb-1 ml-1 transition-colors ${authStatus === 'error' ? 'text-red-500' : authStatus === 'success' ? 'text-green-500' : 'text-gray-700 dark:text-gray-300 group-focus-within:text-blue-500 dark:group-focus-within:text-blue-400'}`}
            >
              Udemy Access Token
            </label>
            ...
            <input
              ...
              className={`w-full bg-white/50 dark:bg-black/20 border rounded-2xl px-5 py-4 text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-gray-600 focus:outline-none transition-all shadow-inner cursor-pointer disabled:opacity-50
                ${
                  authStatus === 'error'
                    ? 'border-red-500/50 focus:border-red-500 focus:ring-2 focus:ring-red-500/20'
                    : authStatus === 'success'
                      ? 'border-green-500 focus:border-green-500 focus:ring-2 focus:ring-green-500/20 shadow-[0_0_15px_rgba(34,197,94,0.2)]'
                      : 'border-gray-200 dark:border-white/10 focus:border-blue-500/50 focus:ring-2 focus:ring-blue-500/20'
                }
              `}
              value={token}
              onChange={(e): void => {
          ...
          <button
            onClick={handleLogin}
            disabled={authStatus === 'validating' || authStatus === 'success'}
            className={`w-full text-white font-bold py-4 px-6 rounded-2xl transition-all transform cursor-pointer mt-2 disabled:opacity-70 disabled:cursor-not-allowed disabled:transform-none
              ${
                authStatus === 'success'
                  ? 'bg-green-500 shadow-[0_0_20px_rgba(34,197,94,0.4)]'
                  : 'bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 shadow-[0_0_20px_rgba(79,70,229,0.3)] hover:shadow-[0_0_30px_rgba(79,70,229,0.6)] hover:-translate-y-0.5'
              }
            `}
          >
      ...
      <div className="absolute bottom-6 text-xs text-gray-400 dark:text-gray-600 font-medium tracking-widest uppercase transition-colors duration-300">
      ...
      {showHelpModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 dark:bg-black/70 backdrop-blur-md transition-opacity">
          <div className="relative bg-white dark:bg-[#111118] border border-gray-200 dark:border-white/10 rounded-[2rem] p-8 sm:p-10 max-w-xl w-full shadow-2xl animate-in fade-in zoom-in-95 duration-300">
            {/* Close Button */}
            <button
              onClick={(): void => setShowHelpModal(false)}
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="flex items-start gap-5 p-5 rounded-2xl bg-gray-50 dark:bg-white/5 border border-gray-100 dark:border-white/5 transition-colors hover:bg-gray-100 dark:hover:bg-white/10">
                ...
                <p className="text-gray-700 dark:text-gray-300 leading-relaxed mt-1.5">
                  Navigate to the{' '}
                  <strong className="text-gray-900 dark:text-white">Application</strong> tab
                  (Chrome/Edge) or{' '}
              ...
              <div className="flex items-start gap-5 p-5 rounded-2xl bg-blue-50 dark:bg-blue-900/10 border border-blue-100 dark:border-blue-500/20 transition-colors">
                ...
                <p className="text-gray-700 dark:text-gray-300 leading-relaxed mt-1.5">
                  Expand <strong className="text-gray-900 dark:text-white">Cookies</strong> and
                  click{' '}
